    ...
    DownloadRowProcessor
{
    ...
    public async Task ProcessAsync(LinkRow row, string outputFolder, CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(row.MainDownloadLink) && !string.IsNullOrWhiteSpace(row.IdName))
        {
            ...
            if (!downloadedFromMain && !string.IsNullOrWhiteSpace(row.AlternativeDownloadLink))
            {
                ...
                await _downloadService.TryDownloadAsync(
        ...
        if (!string.IsNullOrWhiteSpace(row.AlternativeDownloadLink) && !string.IsNullOrWhiteSpace(row.IdName))
        {
            await _downloadService.TryDownloadAsync(row.AlternativeDownloadLink, outputFolder, row.IdName, cancellationToken);
        }
    }
}
